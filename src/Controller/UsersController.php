...
class UsersController extends AppController
{
    ...
    public function requestUser()
    {
        ...
        $this->set(['error' => $message, 'user' => $user, 'error_api' => $api]);
    }
        ...
        if ($this->request->is('post')) {
            ...
            if ($user) {
                if ($user['status']) {
                    ...
                    // test API
                    $result_test_api = true; // $this->testApi($user['id']);
//                    $result_test_api = true;
                    if ($user['role'] == 'medecin') {
                        if (!$result_test_api) {
                            $this->set('id_incorrect', 3);
                        } else {
                            $this->setLockDown(false);
                            $this->proceeLogin($user);
                        }
                    } else {
                        if ($result_test_api)
                            $this->setLockDown(false);
    ...
    private function proceeLogin($user)
    {
    ...
    private
    function messageToSend($username, $userpassword, $role)
    {
        $message = '';
        if ($role == 'medecin') {
     ...
     */
    private
    function sendPassword($username, $usermail, $userpassword, $role)
    {
        $to = $usermail;
     ...
     */
    public
    function changePasswordMedecin()
    {
        $data = $this->request->data;
    ...
    public function hello()
    {
        ...
        $html =
         ...
         $subject = 'Votre pdf';
    //        $message = $this->messageToSend($username, $userpassword, $role);
         $mail = $this->Email->send('wyzdevsa@gmail.com', $subject, 'message', WWW_ROOT . DS . "Brochure.pdf");
         $this->set('mail',$mail);
